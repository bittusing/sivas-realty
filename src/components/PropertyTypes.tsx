...
export default function PropertyTypes() {
  return (
    <section className="py-20 bg-gradient-to-b from-bg-light via-accent/30 to-bg-light">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {types.map((type) => (
            <Link
              key={type.title}
              href={type.href}
              className="group bg-white rounded-2xl shadow-lg p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 border border-transparent hover:border-secondary/30"
            >
              <div className="w-16 h-16 rounded-xl gradient-gold flex items-center justify-center mb-6 group-hover:scale-110 transition-transform shadow-lg shadow-secondary/20">
                <type.icon className="w-8 h-8 text-white" />
              </div>
              <h3 className="text-xl font-bold text-text-dark mb-3 font-[family-name:var(--font-heading)] group-hover:text-secondary transition-colors">
                {type.title}
              </h3>
